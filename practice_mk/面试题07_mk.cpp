...
class Solution {
 public:
  ...
  void cutTree(vector<int> left_order, vector<int> right_order,
               vector<int>& preorder, vector<int>& inorder, int level,
               TreeNode* tree) {
    ++level;
    // left
    ...
    if (left_order.size() != 0) {
      ...
      if (left_order.size() > 1)
        cutTree(nleft_order, nright_order, preorder, inorder, level, left_tree);
    ...
    if (right_order.size() != 0) {
      ...
      if (right_order.size() > 1)
        cutTree(nleft_order, nright_order, preorder, inorder,
                level + left_order.size(), right_tree);
    } else {
      tree->right = NULL;
    }
